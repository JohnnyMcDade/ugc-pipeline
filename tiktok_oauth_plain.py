#!/usr/bin/env python3
"""TikTok Content Posting API — OAuth helper (PKCE plain-method variant).

Identical to tiktok_oauth.py EXCEPT it uses PKCE method `plain` instead
of `S256`. RFC 7636 §4.2 defines plain: the code_challenge sent to the
authorize endpoint IS literally the same string as the code_verifier
sent later to the token endpoint — no SHA-256, no base64url encoding.
The server stores the challenge and on exchange just string-compares.

Why try this: TikTok rejected our S256 PKCE three times running with
provably correct math (verified against openssl). The plain method
removes any hash/encoding ambiguity — if TikTok accepts this but not
S256, the bug is on their hash-comparison path, not ours.

ALL OTHER BEHAVIOR IS IDENTICAL to tiktok_oauth.py:
  - http://localhost:8080/callback redirect URI
  - 64-char verifier from base64url-strict alphabet
  - Sandbox/production lanes via --sandbox flag
  - Atomic .env write + post-write verification

Note: RFC 7636 *recommends* S256 over plain (plain has weaker security
properties — anyone who can read the authorize request can replay the
token exchange). For production you should prefer S256 once it works.
This script is a diagnostic step, not a long-term solution.

Usage:
    python tiktok_oauth_plain.py sharpguylab
    python tiktok_oauth_plain.py sharpguylab --sandbox
...
def _generate_pkce() -> tuple[str, str]:
    """Generate a PKCE pair using the `plain` method (RFC 7636 §4.2).

    Returns (code_verifier, code_challenge) — and for the plain method
    these two values are IDENTICAL. No SHA-256, no base64url encoding;
    the challenge IS the verifier. The verifier still has to satisfy
    RFC 7636 §4.1 (43-128 chars from the unreserved set), so we keep
    the same generator as the S256 variant.
    """
    verifier = "".join(secrets.choice(PKCE_VERIFIER_ALPHABET) for _ in range(PKCE_VERIFIER_LENGTH))
    challenge = verifier  # PKCE plain method: challenge == verifier
...
def _build_authorize_url(client_key: str, state: str, code_challenge: str) -> str:
    qs = urllib.parse.urlencode({
        ...
        "state": state,
        # PKCE — plain method: challenge is the verifier itself.
...
def run_oauth(account_handle: str, *, sandbox: bool = False) -> None:
    ...
    # For the plain method, challenge IS the verifier — same length, same charset.
    ...
    assert code_challenge == code_verifier, "plain-method challenge must equal verifier"
    ...
    print(f"  PKCE:         plain (challenge == verifier — no hashing)")
    print()
    print(f"  code_verifier:  {code_verifier}")
    ...
    print(f"    length:       {len(code_challenge)} chars (identical to verifier)")
    print()
    print(f"  Verify trivially: challenge == verifier  →  {code_verifier == code_challenge}")
    print()
