#!/usr/bin/env python3
"""TikTok Content Posting API — OAuth helper (127.0.0.1 variant).

Identical to tiktok_oauth.py EXCEPT the redirect URI uses 127.0.0.1
instead of the "localhost" hostname, and the callback server binds to
the same. Some OAuth providers (and possibly TikTok in certain app-tier
configurations) require the redirect_uri host to resolve to the literal
IPv4 loopback address rather than a hostname that *could* resolve to
::1 or another address.

ALL OTHER BEHAVIOR IS IDENTICAL to tiktok_oauth.py:
  - PKCE S256 with the base64url-strict verifier alphabet
  - Sandbox/production lanes via --sandbox flag
  - Atomic .env write + post-write verification

PREREQUISITE: add `http://127.0.0.1:8080/callback` to the TikTok dev
app's allowed redirect URIs alongside (or instead of) the localhost one.

Usage:
    python tiktok_oauth_127.py sharpguylab
    python tiktok_oauth_127.py sharpguylab --sandbox

PREREQUISITES on the TikTok dev console for your app:
  1. `http://127.0.0.1:8080/callback` is in the app's allowed redirect URIs.
     If you previously only added the `localhost` form, you'll need to add
     this one too — TikTok treats them as distinct strings, not as DNS
     aliases.
...
def _start_callback_server() -> socketserver.TCPServer:
    ...
    try:
        # Bind to 127.0.0.1 explicitly — some OAuth providers require the
        # redirect_uri host to resolve to the literal IPv4 loopback (not
        # the "localhost" hostname), and the listener must match.
        server = socketserver.TCPServer(("127.0.0.1", CALLBACK_PORT), _CallbackHandler)
    except OSError as e:
        raise SystemExit(
            f"✗ Could not bind 127.0.0.1:{CALLBACK_PORT}: {e}\n"
