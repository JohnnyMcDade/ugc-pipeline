#!/usr/bin/env python3
"""TikTok Content Posting API — OAuth helper (43-char verifier variant).

Identical to tiktok_oauth.py EXCEPT the PKCE code_verifier is exactly
43 characters — RFC 7636's MINIMUM. If TikTok's PKCE validator has an
off-by-one or rejects anything except the spec floor, this catches it.

Challenge is still BASE64URL(SHA256(verifier)) = 43 chars regardless of
verifier length (SHA-256 output is fixed).
...
Usage:
    python tiktok_oauth_min43.py sharpguylab
    python tiktok_oauth_min43.py sharpguylab --sandbox
...
PKCE_VERIFIER_LENGTH = 43  # RFC 7636 MINIMUM — testing if TikTok wants the floor
