#!/usr/bin/env python3
"""TikTok Content Posting API — OAuth helper (128-char verifier variant).

Identical to tiktok_oauth.py EXCEPT the PKCE code_verifier is exactly
128 characters — RFC 7636's MAXIMUM. Opposite hypothesis from
tiktok_oauth_min43.py: if TikTok expects a longer verifier than the
default 64 chars, the ceiling catches it.

Challenge is still BASE64URL(SHA256(verifier)) = 43 chars (SHA-256
output is fixed regardless of input length).
...
Usage:
    python tiktok_oauth_max128.py sharpguylab
    python tiktok_oauth_max128.py sharpguylab --sandbox
...
PKCE_VERIFIER_LENGTH = 128  # RFC 7636 MAXIMUM — testing if TikTok wants the ceiling
